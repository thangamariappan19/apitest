...
export class CurrencyListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("currency?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.currencyMasterList;
...
dataTableDefault() {
    ...
    this.columnsCopy = [
      { key:'currencyCode', title: 'Currency Code', width: '15%' }, // orderBy: 'desc' },
      { key:'currencyName', title: 'Currency Name', width: '15%' },
      { key:'currencySymbol',title:'Currency Symbol',width:'15%'},
      { key:'interDescription',title:'Description',width:'15%'},
      { key:'decimalPlaces',title:'Decimal Places',width:'20%'},
      { key:'currencyType',title:'Currency Type',width:'10%'},
  ...
  //     x => x.currencyCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.currencyName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.currencySymbol.toLowerCase().includes(name.toLowerCase()) ||
  //       x.interDescription.toLowerCase().includes(name.toLowerCase()) ||
  //       x.decimalPlaces.toString().includes(name.toString()) ||
  //       x.currencyType.toLowerCase().includes(name.toLowerCase()) 
  //   );
  // }
  // getRoles() {
  //   this.currency = null;
  ...
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //          // this.currency = data.currencyMasterList;
  //           this.tblData = data.currencyMasterList;
  //           this.tblDataCopy = data.currencyMasterList;
