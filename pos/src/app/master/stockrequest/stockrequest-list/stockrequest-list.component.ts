...
@Component({
  selector: 'app-stockrequest-list',
  templateUrl: './stockrequest-list.component.html',
  styleUrls: ['./stockrequest-list.component.css'],
...
export class StockrequestListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("StockRequest?StoreID="+this.user_details.storeID + "&limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key:'documentNo', title: 'Document No', width: '35%' }, // orderBy: 'desc' },
      { key:'documentDate', title: 'Document Date', width: '30%' },
      { key:'status',title:'Status',width:'30%'},
      { key: 'active', title: 'Active', width: '5%' }, //headerActionTemplate: this.levelHeaderActionTemplate },
      { key: '', title: '' },
    ];
  ...
  //   this.stockrequest_Filter = this.stockrequest_List;
  // }

  // StockRequestList() {
  //   this.stockrequest_List = null;
  //   this.common.showSpinner();
  //   this.api.getAPI("StockRequest?storeId="+this.user_details.storeID )
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           //this.stockrequest_List = data.responseDynamicData;
  //          // this.stockrequest_Filter = this.stockrequest_List;
  //           this.tblData = data.responseDynamicData;
  ...
  //     this.stockrequest_Filter = this.stockrequest_Filter.filter(x => x.documentNo.toLowerCase().includes(documentNo.toLowerCase()));

  //   if (documentDate != null && documentDate != "" && documentDate != "NaN-NaN-NaN")
  //   this.stockrequest_Filter = this.stockrequest_Filter.filter(x => this.common.toYMDFormat(new Date(x.documentDate)).toString().includes(documentDate));
    
  //     if (status != null && status != "")
  //     this.stockrequest_Filter = this.stockrequest_Filter.filter(x => x.status.toLowerCase().includes(status.toLowerCase()));
