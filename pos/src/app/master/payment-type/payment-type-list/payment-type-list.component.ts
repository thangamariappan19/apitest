...
import { MPaymentMaster } from 'src/app/models/m-payment-master';
...
export class PaymentTypeListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("paymenttype?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.paymentTypeMasterList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'paymentCode', title: 'Payment Type Code', width: '20%' },
      { key: 'paymentName', title: 'Payment Type Name', width: '20%' },
      { key: 'paymentType', title: 'Payment Mode', width: '20%' },
      { key: 'countryCode', title: 'Country', width: '20%' },
  ...
  //     x => x.paymentCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.paymentName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.paymentType.toLowerCase().includes(name.toLowerCase()) ||
  ...
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           //this.payment_list = new Array<MPaymentType>();
  //           //this.payment_list = data.paymentTypeMasterList;
  //           this.tblData = data.paymentTypeMasterList;
  //           this.tblDataCopy = data.paymentTypeMasterList;
  //         } else {
  //           let msg: string = data != null
  //             && data.displayMessage != null
  //             && data.displayMessage != "" ? data.displayMessage : "Failed to retrieve Data.";
