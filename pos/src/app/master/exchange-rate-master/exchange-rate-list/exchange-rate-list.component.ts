...
export class ExchangeRateListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("exchangerates?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'exchangeRatesCode', title: 'Code', width: '20%' }, // orderBy: 'desc' },
      { key: 'exchangeRatesName', title: 'Name', width: '20%' },
      { key:'exchangeRateDate',title:'Date',width:'20%'},
      { key:'baseCurrency',title:'Base Currency',width:'20%'},
  ...
  //     x => x.exchangeRatesCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.exchangeRatesName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.exchangeRateDate.toString().includes(name.toLowerCase()) ||
  //       x.baseCurrency.toLowerCase().includes(name.toLowerCase())
  //   );
  // }

  // getExchangeRateList(){
  //   this.exchangeRateList = null;
  //   this.exchangeRateFilter = null;
  //   this.common.showSpinner();
  //   this.api.getAPI("exchangerates")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           //this.json = new Array<MCountryMaster> ();
  //          // this.exchangeRateList = data.responseDynamicData;
  //           //this.exchangeRateFilter = this.exchangeRateList
  //           this.tblData = data.responseDynamicData;
