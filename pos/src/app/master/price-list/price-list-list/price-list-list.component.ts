...
export class PriceListListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("pricelist?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
...
this.dataTableDefault();
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'priceListCode', title: 'Price List Code', width: '20%' }, // orderBy: 'desc' },
      { key: 'priceListName', title: 'Price List Name', width: '20%' },
      { key: 'currencyName', title: 'Currency', width: '15%' },
      { key: 'conversionFactore', title: 'Conversion Factor', width: '15%' },
     // { key: 'basePriceListID', title: 'Base Price-List', width: '15%' },
  ...
  onChange(name: string): void {
    this.tblData = this.tblDataCopy.filter(
      x => x.priceListCode.toLowerCase().includes(name.toLowerCase()) ||
        x.priceListName.toLowerCase().includes(name.toLowerCase()) ||
        x.currencyName.toLowerCase().includes(name.toLowerCase()) ||
        x.conversionFactore.toString().includes(name.toString()) ||
       // x.basePriceListID.toLowerCase().includes(name.toLowerCase()) ||
  ...
  getPriceList() {
    ...
    this.common.showSpinner();
    this.api.getAPI("pricelist")
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
           // this.price_List = data.responseDynamicData;
