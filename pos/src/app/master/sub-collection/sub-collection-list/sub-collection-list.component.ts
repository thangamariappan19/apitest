...
@Component({
  selector: 'app-sub-collection-list',
  templateUrl: './sub-collection-list.component.html',
  styleUrls: ['./sub-collection-list.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SubCollectionListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("SubCollection?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.subCollectionMasterList;
  ...
  dataTableDefault() {
    ...
    // this.columnsCopy = [
    //   { key: '', title: 'ID', width: '10%' },
    //   { key: 'collectionCode', title: 'Collection Code', width: '40%' },
    //   { key: 'collectionName', title: 'Collection Name', width: '40%' },
    //   { key: 'active', title: 'Active', width: '10%' },
    //   { key: '', title: '' },
    // ];
    this.columnsCopy = [
      { key: 'collectionCode', title: 'Collection Code', width: '50%' },
      { key: 'collectionName', title: 'Collection Name', width: '50%' },
      { key: '', title: '' },
    ];
  ...
  //     x => x.collectionCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.collectionName.toLowerCase().includes(name.toLowerCase())
  //   );
  // }
  // getCollectionList() {
  //   this.collectionsList = null;
  //   this.collection_Filter = null;
  //   this.common.showSpinner();
  //   this.api.getAPI("SubCollection")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           //this.collectionsList = data.subCollectionMasterList;
  //           //this.collection_Filter = this.collectionsList;
  //           this.tblData = data.subCollectionMasterList;
  //           this.tblDataCopy = data.subCollectionMasterList;
  //           for (let item of this.tblData) {
