...
export class ShiftListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("shift?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.shiftList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'countryCode', title: 'Country Code', width: '35%' }, // orderBy: 'desc' },
      { key: 'countryName', title: 'Country Name', width: '35%' },
  ...
  //     x => x.countryCode.toLowerCase().includes(name.toLowerCase()) ||
  ...
  //   this.common.showSpinner();
  //   this.api.getAPI("shift")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //          // this.shiftList = data.shiftList;
  //          // this.shift_Filter = this.shiftList;
  //           this.tblData = data.shiftList;
  //           this.tblDataCopy = data.shiftList;
  //          // for (let item of this.shift_Filter) {
  ...
  // }
  // filter() {
  //   let countryCode = this.myForm.get('countryCode').value;
  //   let countryName = this.myForm.get('countryName').value;

  //   this.shift_Filter = this.shiftList;

  //   if (countryCode != null && countryCode != "")
  //      this.shift_Filter = this.shift_Filter.filter(x => x.countryCode.toLowerCase().includes(countryCode.toLowerCase()));

  //   if (countryName != null && countryName != "")
  //     this.shift_Filter = this.shift_Filter.filter(x => x.countryName.toLowerCase().includes(countryName.toLowerCase()));
