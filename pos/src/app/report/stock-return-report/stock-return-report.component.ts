...
@Component({
  ...
  styleUrls: ['./stock-return-report.component.css']
})
export class StockReturnReportComponent implements OnInit {
  ...
  generatePDF(): void {
    console.log(this.stockretunList1);
    if (this.reportselectedvalue == "Stock Return Header") {
        ...
        content: [
          ...
          // },
          {
            table: {
              ...
              headerRows: 1,
              //widths: ['*', '*', '*', '*', '*' , '*'],
              widths: ['12.6%', '12.6%', '12.6%', '12.6%', '12.6%', '12.6%', '12.6%'],
              body: [
                ['DocumentNo', 'Date', 'Quantity', 'User', 'ToLoc', 'FromLoc', 'SendDate'],
                // ['888','hhh','hhh','uuu','uygg'],formatDate(this.business_date, 'dd-MM-yyyy', 'en')
                ...this.stockretunList.map(p => ([p.documentNo,formatDate(p.documentDate, 'dd-MM-yyyy', 'en'), p.totalQuantity, p.remarks, p.toWareHouseCode, p.storeCode,formatDate(p.createOn, 'dd-MM-yyyy', 'en')])),
    ...
    if (this.reportselectedvalue == "Stock Return Details") {
      this.documentDefinition = {
        info: {
          // title: this.pdfData.title,
        ...
        content: [
          ...
          // },
          {
            table: {
              ...
              widths: ['12.6%', '12.6%', '12.6%', '12.6%', '12.6%', '12.6%', '12.6%'],
              body: [
                ['Date', 'Item', 'ToLoc', 'Quantity', 'FromLoc', 'DocumentNo', 'SendDate'],
                // ['888','hhh','hhh','uuu','uygg'],
                ...this.stockretunList1.map(p => ([formatDate(p.documentDate, 'dd-MM-yyyy', 'en'), p.skuCode, p.skuName, p.quantity, p.remarks, p.brand,formatDate(p.createOn, 'dd-MM-yyyy', 'en')])),
  ...
  getstockreturn() {
    ...
    this.api.getAPI("StockReturnReport?fromDate=" + this.fromdate + "&toDate=" + this.todate + "&reportselectedvalue=" + this.reportselectedvalue + "")
      .subscribe((data) => {
        setTimeout(() => {
          console.log(data);
          if (data != null) {
            if (this.reportselectedvalue == "Stock Return Header") 
            {
              this.stockretunList = data.stockReturnHeaderList;
            }
            if (this.reportselectedvalue == "Stock Return Details") 
            {
              this.stockretunList1 = data.stockReturnDetailsList;
            }



            //  this.DocumentNo = data.stockReturnHeaderList[0].documentNo;
            //  this.Date = data.stockReturnHeaderList[0].documentDate;
            //  this.quantity = data.stockReturnHeaderList[0].totalQuantity;
            //  this.user = data.stockReturnHeaderList[0].remarks;
            //  this.ToLoc = data.stockReturnHeaderList[0].toWareHouseCode;
            //  this.FromLoc = data.stockReturnHeaderList[0].storeCode;
            //  this.SendDate = data.stockReturnHeaderList[0].createOn;
            this.getData();
