...
@Component({
  selector: 'app-sales-return-invoice-report',
  templateUrl: './sales-return-invoice-report.component.html',
  styleUrls: ['./sales-return-invoice-report.component.css']
})
export class SalesReturnInvoiceReportComponent implements OnInit {
  ...
  {
    this.common.showSpinner();
    this.api.getAPI("DetailedSalesInvoice?InvoiceNo=" + this.bindate + "&StoreID="+ this.user_details.storeID + "&Mode="+ 2)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1)
          {
        ...
        [
               ...
               [
              {
                ...
                table: {
                    ...
                    body: [
                        [
                          {fillColor: '#dddddd',bold: true,text: 'Item Code'},
                          {fillColor: '#dddddd',bold: true,text: 'Return QTY'},
                          {fillColor: '#dddddd',bold: true,text: 'Unit Price'}
                        ],
                        ...this.ReportList.map(p => ([p.itemCode, p.returnQty.toFixed(this.decimal_places),p.unitPrice.toFixed(this.decimal_places)])),
                ...
                body: [
                  [
                    ...
                    },
                    {
                      ...
                      text:'Gross Amt\t\t:\t'+this.ReportList.reduce((sum,p)=> sum + (p.grossAmt), 0).toFixed(this.decimal_places)+'\n\nDisc Amt\t\t\t:\t'+ this.ReportList.reduce((sum,p)=> sum + (p.discountAmount).toFixed(this.decimal_places), 0)+'\n\n Sale Tax\t\t\t:\t'+ this.ReportList.reduce((sum,p)=> sum + (p.taxAmount), 0).toFixed(this.decimal_places)
                      +'\n\n Net Amt\t\t\t:\t'+this.ReportList.reduce((sum,p)=> sum + (p.cashReceivedAmount), 0).toFixed(this.decimal_places)
                      +'\n\n Amount Returned:\t'+this.ReportList.reduce((sum,p)=> sum + (p.totalReturnAmount), 0).toFixed(this.decimal_places)
