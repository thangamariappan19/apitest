...
@Component({
  selector: 'app-coupon-receipt-edit',
  templateUrl: './coupon-receipt-edit.component.html',
  styleUrls: ['./coupon-receipt-edit.component.css']
})
export class CouponReceiptEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.GetCouponReceiptHeaderData1();
    this.SelectCouponReceiptDetails();
  }
  GetCouponReceiptHeaderData1()
  {
    this.id = this.activatedRoute.snapshot.paramMap.get('id');
    this.common.showSpinner();
    this.api.getAPI("CouponReceipt?ID="+this.id).subscribe((data) => {
      setTimeout(() => {
        if (data != null && data.statusCode != null && data.statusCode == 1) {
          this.mCouponReceiptMaster=data.couponReceiptHeaderRecord;
          this.Couponid=this.mCouponReceiptMaster.couponCode;
         
          this.myForm.controls['Currentlocation'].setValue(this.mCouponReceiptMaster.currentLocation);
          this.myForm.controls['FromSerialNum'].setValue(this.mCouponReceiptMaster.fromSerialNum);
  ...
  SelectCouponReceiptDetails()
  {
    this.id = this.activatedRoute.snapshot.paramMap.get('id');
    this.common.showSpinner();
    this.api.getAPI("CouponReceipt?ID="+ this.id +"&details="+'details').subscribe((data) => {
      setTimeout(() => {
        if (data != null && data.statusCode != null && data.statusCode == 1) {
          this.mCouponReceiptMaster.couponReceiptDetailsList=data.couponReceiptHeaderRecord.couponReceiptDetailsList;
          this.tablelist =this.mCouponReceiptMaster.couponReceiptDetailsList;
          this.CouponReceiptDetailsList1=this.mCouponReceiptMaster.couponReceiptDetailsList;
  ...
  btnGenerate_Click()
  {
    ...
    if (this.FromSerialNum != "" && this.ToSerialNum != "")
      ...
      this.common.showMessage("warn", "Must select FromSerialNum and ToSerialNum..");
    }
//     this.ToSerialNum=this.myForm.get('ToSerialNum').value;
//     this.FromSerialNum=this.myForm.get('FromSerialNum').value;
//     if (((this.FromSerialNum != "") && (this.ToSerialNum != ""))) 
...
btnUpdate_Click()
{
  ...
  this.mCouponReceiptMaster.id=this.id;
...
set Couponid(value:string) 
  {
    let cid=this.CouponCodeList.find(x=> x.couponName==value);
    this.myForm.controls['CouponCode'].setValue(cid.id);
    this.CouponCode=cid.couponCode;
  }
}
