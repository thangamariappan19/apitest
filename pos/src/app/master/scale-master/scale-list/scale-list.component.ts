...
export class ScaleListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("scale?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'scaleCode', title: 'Scale Code', width: '40%' },
      { key: 'scaleName', title: 'Scale Name', width: '40%' },
  ...
  //     x => x.scaleCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.scaleName.toLowerCase().includes(name.toLowerCase())
  //   );
  // }
  // getStateList() {
  //   this.scaleMasterList = null;
  //   this.scaleMaster_Filter = null;
  //   this.common.showSpinner();
  ...
  //           //this.scaleMaster_Filter = this.scaleMasterList;
  //           this.tblData = data.responseDynamicData;
