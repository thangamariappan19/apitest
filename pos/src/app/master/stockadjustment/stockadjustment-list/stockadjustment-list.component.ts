...
@Component({
  selector: 'app-stockadjustment-list',
  templateUrl: './stockadjustment-list.component.html',
  styleUrls: ['./stockadjustment-list.component.css'],
...
export class StockadjustmentListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("StockAdjustment?StoreID="+this.user_details.storeID + "&limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.stockAdjustmentList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key:'documentNumber', title: 'Document No', width: '35%' }, // orderBy: 'desc' },
      { key:'documentDate', title: 'Document Date', width: '30%' },
      { key:'createdByUserName',title:'User Name',width:'30%'},
  ...
  //     x => x.documentNumber.toLowerCase().includes(name.toLowerCase()) ||
  //       //x.documentDate.toLowerCase().includes(name.toLowerCase()) ||
  //       x.createdByUserName.toLowerCase().includes(name.toLowerCase()) 
  //   );
  // }

  // StockAdjustmentList() {
  //   this.stockadjustment_List = null;
  //   this.common.showSpinner();
  //   this.api.getAPI("StockAdjustment?storeId="+this.user_details.storeID)
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           this.stockadjustment_List = data.stockAdjustmentList;
  //          // this.stockadjustment_Filter = this.stockadjustment_List;
  //           this.tblData = data.stockAdjustmentList;
  //           this.tblDataCopy = data.stockAdjustmentList;
  //         } else {
  //           let msg: string = data != null
  //             && data.displayMessage != null
  //             && data.displayMessage != "" ? data.displayMessage : "Failed to retrieve Data.";
  ...
  // ngOnInit() {
  // }
  // getViewStoctAdjustment(id : number) {
  //   /*var target = event.target || event.srcElement || event.currentTarget;
  //   var idAttr = target.attributes.id;
  //   var value = idAttr.nodeValue;*/
    
  //   let arr = this.tblData.filter(x => x.id == id);
  //   if(arr != null && arr.length > 0){
  //     console.log(arr[0]);
  //     localStorage.setItem('ViewStockAdjustment', JSON.stringify(arr[0]));
  //     //console.log(this.tempstockadjustment);
  //     this.router.navigate(['stockadjustment-view']);
  //   }
  //   else
  //   {
  //     this.common.showMessage('warn', "Invalid Stock Adjustment Data.");
  //   }
  ...
  //     this.stockadjustment_Filter = this.stockadjustment_Filter.filter(x => x.documentNumber.toLowerCase().includes(documentNo.toLowerCase()));

  //   if (documentDate != null && documentDate != "" && documentDate != "NaN-NaN-NaN")
  //   this.stockadjustment_Filter = this.stockadjustment_Filter.filter(x => this.common.toYMDFormat(new Date(x.documentDate)).toString().includes(documentDate));
    
  //   //if (userName != null && userName != "")
  //    // this.stockadjustment_Filter = this.stockadjustment_Filter.filter(x => x.createBy.toLowerCase().includes(userName.toLowerCase()));
