...
export class ReasonListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("reason?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.reasonMasterList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'reasonCode', title: 'Reason Code', width: '30%' },
      { key: 'reasonName', title: 'Reason Name', width: '30%' },      
  ...
  onChange(name: string): void {
    this.tblData = this.tblDataCopy.filter(
      x => x.reasonCode.toLowerCase().includes(name.toLowerCase()) ||
        x.reasonName.toLowerCase().includes(name.toLowerCase()) ||
  ...
  getReasonList() {
    ...
    this.common.showSpinner();
    this.api.getAPI("reason")
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            //this.reason_List = data.reasonMasterList;
            //this.json = data.companySettingList;
            //.log(this.json);
            this.tblData = data.reasonMasterList;
            this.tblDataCopy = data.reasonMasterList;
          } else {
            let msg: string = data != null
              && data.displayMessage != null
              && data.displayMessage != "" ? data.displayMessage : "Failed to retrieve Data.";
