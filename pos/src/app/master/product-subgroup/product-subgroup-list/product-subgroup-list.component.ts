...
@Component({
  selector: 'app-product-subgroup-list',
  templateUrl: './product-subgroup-list.component.html',
  styleUrls: ['./product-subgroup-list.component.css']
})
export class ProductSubgroupListComponent implements OnInit {
  productsubgroupList: Array<MProductsubgroupMaster>;
  productsubgroup_Filter: Array<MProductsubgroupMaster>;
  myForm: FormGroup;
  ...
  private getData(): void {
    ...
    this.api.getAPI("productsubgroup?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.productGroupList;
  ...
  clear_controls() {
    ...
    this.productsubgroup_Filter = this.productsubgroupList;
  }
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'id', title: 'Product ID', width: '25%' }, // orderBy: 'desc' },
      { key: 'productGroupCode', title: 'Product Code', width: '25%' },
      { key: 'productGroupName', title: 'Product Name', width: '25%' },
  ...
  onChange(name: string): void {
    this.tblData = this.tblDataCopy.filter(
      x => x.productGroupCode.toLowerCase().includes(name.toLowerCase()) ||
      x.productGroupName.toLowerCase().includes(name.toLowerCase())
    );
  }
  ...
  getProductSubGroupList() {
    ...
    this.productsubgroup_Filter = null;
    this.common.showSpinner();
    this.api.getAPI("productsubgroup")
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
           // this.productsubgroupList = data.productGroupList;
            //this.productsubgroup_Filter = this.productsubgroupList;
            this.tblData = data.productGroupList;
            this.tblDataCopy = data.productGroupList;
           //for (let item of this.productsubgroup_Filter) {
  ...
  filter() {
    let productGroupCode = this.myForm.get('productGroupCode').value;
    let productGroupName = this.myForm.get('productGroupName').value;

    this.productsubgroup_Filter = this.productsubgroupList;

    if (productGroupCode != null && productGroupCode != "")
       this.productsubgroup_Filter = this.productsubgroup_Filter.filter(x => x.productGroupCode.toLowerCase().includes(productGroupCode.toLowerCase()));

    if (productGroupName != null && productGroupName != "")
      this.productsubgroup_Filter = this.productsubgroup_Filter.filter(x => x.productGroupName.toLowerCase().includes(productGroupName.toLowerCase()));
