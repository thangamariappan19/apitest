...
@Component({
  selector: 'app-retail-settings-list',
  templateUrl: './retail-settings-list.component.html',
  styleUrls: ['./retail-settings-list.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RetailSettingsListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("retailsettings?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'retailCode', title: 'Retail Code', width: '25%' }, // orderBy: 'desc' },
      { key: 'retailName', title: 'Retail Name', width: '25%' },
      { key: 'changeAmountCurrency', title: 'Change Amount Currency', width: '25%' },
  ...
  //     x => x.retailCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.retailName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.changeAmountCurrency.toString().includes(name.toString())
       
       
  //   );
  // }

  // getRetailSettingsList() {
  //   this.retailSettingList = null;
  //   this.common.showSpinner();
  //   this.api.getAPI("retailsettings")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //          // this.retailSettingList = data.responseDynamicData;
  //           //this.retailSettingListFilter = this.retailSettingList;
