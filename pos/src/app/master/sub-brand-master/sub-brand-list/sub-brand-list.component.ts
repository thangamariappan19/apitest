...
@Component({
  selector: 'app-sub-brand-list',
  templateUrl: './sub-brand-list.component.html',
  styleUrls: ['./sub-brand-list.component.css'],
...
export class SubBrandListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("subbrand?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.subBrandList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'brandID', title: 'Brand Code', width: '50%' },
      { key: 'brandName', title: 'Brand Name', width: '50%' },
      { key: '', title: '' },
    ];
  ...
  //     x => x.brandID.toString().includes(name.toString()) ||
  ...
  //   this.common.showSpinner();
  //   this.api.getAPI("subbrand")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           //this.subBrandList = data.subBrandList;
  //           //this.brandFilter = this.subBrandList;
  //           this.tblData = data.subBrandList;
  //           this.tblDataCopy = data.subBrandList;
