...
export class CityListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("CityMaster?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.cityList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'cityCode', title: 'City Code', width: '25%' }, // orderBy: 'desc' },
      { key: 'cityName', title: 'City Name', width: '25%' },  
      { key: 'stateName', title: 'State Name', width: '25%' },  
     // { key: 'remarks', title: 'Remarks', width: '25%' },    
      { key: 'active', title: 'Active', width: '25%' },
      { key: '', title: ''},
    ];
...
//     this.cityList = null;
//     this.common.showSpinner();
//     this.api.getAPI("CityMaster")
//       .subscribe((data) => {
//         setTimeout(() => {
//           if (data != null && data.statusCode != null && data.statusCode == 1) {
//             this.tblData = data.cityList;
//             this.tblDataCopy = data.cityList;
//           } else {
//             let msg: string = data != null
//               && data.displayMessage != null
//               && data.displayMessage != "" ? data.displayMessage : "Failed to retrieve Data.";
...
//   //clear_controls() {
//    // this.cityMasterList_Filter = this.cityList;
...
//       x => x.cityCode.toLowerCase().includes(name.toLowerCase()) ||
//         x.cityName.toLowerCase().includes(name.toLowerCase()) ||
