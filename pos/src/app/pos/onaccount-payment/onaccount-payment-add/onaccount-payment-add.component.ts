...
@Component({
  selector: 'app-onaccount-payment-add',
  templateUrl: './onaccount-payment-add.component.html',
  styleUrls: ['./onaccount-payment-add.component.css']
})
export class OnaccountPaymentAddComponent implements OnInit {
  myForm: FormGroup;
  Type:any;
  ...
  { 
    ...
    if (this.UserInfo == null) {
     ...
     this.Currencyid=this.UserInfo.currencyID;
    this.createForm();
  }
  ...
  this.paymentAmountbillAmount=0.00;
          ...
          if (data != null && data.statusCode != null && data.statusCode == 1)
           {
            ...
            this.ChangeCurrency=this.UserInfo.currencyCode;
            this.PaymentCurrency=this.UserInfo.currencyCode;
           } 
          else {
            let msg: string =
              data != null &&
              ...
              if (this.UserInfo.storeCode != null && this.UserInfo.storeCode != "")
                {
                  ...
                  if (StoreWiseList != null && StoreWiseList.length > 0)
                  {
                    ...
                    this.myForm.controls['CustomerCode'].setValue(StoreWiseList[0].customerCode);
                    this.myForm.controls['PaymentCurrency'].setValue(this.Currencyid);
                    this.myForm.controls['ChangeCurrency'].setValue(this.Currencyid);
                    this.myForm.controls['CardPaymentCurrency'].setValue(this.Currencyid);
                    this.myForm.controls['InvoiceNo'].setValue(this.SearchString);
                    this.myForm.controls['ChangeCurrency'].disable();
                   // CardPaymentCurrency
                  }
...
CalculatePayments()
{
      ...
      {
          this.PayingAmount =  this.paymentAmountbillAmount;
          ...
          if ((this.BalanceToPay > 0)) {
              ...
              // this.myForm.controls['BalencetoPay'].setValue();
              this.myForm.controls['CashReceivedAmount'].setValue(this.BalanceToPay.toFixed(this.decimal_places));
              this.myForm.controls['CardRecievedAmount'].setValue(this.BalanceToPay.toFixed(this.decimal_places));
          }
          else {
...
AddCashPayment()
{
        //  if(this.OnAccountpaymentpendingList!=null)
        //  {
           if(this.paymentAmountbillAmount == "")
            {
...
AddCardPayment()
{
    ...
    let BalanceToPay: any = this.balanceAmount;
    let NewReceivedAmount: any = this.returnAmount;
...
ResetAll()
 {
          ...
          this.paymentAmountbillAmount=0;
          this.balanceAmount=0;
          this.returnAmount=0;
...
SavePayment()
{
  ...
  this.objOnAccountPayment.billingAmount = this.paymentAmountbillAmount;
  this.objOnAccountPayment.receivedAmount = ReceivedAmount;
  this.objOnAccountPayment.returnAmount = this.returnAmount;
  ...
  mOnAccountPayment.onAccountPaymentRecord= this.objOnAccountPayment;
      this.common.showSpinner();
            ...
            this.common.showMessage('warn','PaymentDetails are Missing !.');
          }
}
SelectItem_checkedChanged(item: MOnAccountInvoiceWisePayment)
{
  ...
  if (this.OnAccountpaymentpendingList.length > 0)
  {
      ...
      {
        this.paymentAmountbillAmount=SelectedList.reduce((sum,x) => sum + x.paidAmount,0).toFixed(3);
        this.balanceAmount=SelectedList.reduce((sum,x) => sum + x.paidAmount,0).toFixed(3);
        this.returnAmount=0;
        //  this.myForm.controls['PayingBillAmount'].setValue();
        //  this.myForm.controls['BalencetoPay'].setValue();
        //  this.myForm.controls['ReturnAmount'].setValue('0');
...
close()
{
  if (confirm("Are You Sure You want to close the On Account Payment?")) 
  {
