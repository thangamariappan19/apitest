...
@Component({
  selector: 'app-manager-override-list',
  templateUrl: './manager-override-list.component.html',
  styleUrls: ['./manager-override-list.component.css']
})
export class ManagerOverrideListComponent implements OnInit {
  manager_Override_List: Array<MManagerOverride>;
  manager_Override_ListFilter: Array<MManagerOverride>;
  myForm: FormGroup;
  ...
  private getData(): void {
    ...
    this.api.getAPI("ManagerOverride?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
...
this.dataTableDefault();
  ...
  clear_controls() {
    ...
    this.manager_Override_ListFilter = this.manager_Override_List;
  }
*/
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'code', title: 'Manager Override Code', width: '40%' }, // orderBy: 'desc' },
      { key: 'name', title: 'Manager Override Name', width: '40%' },
  ...
  /*onChange(name: string): void {
    this.tblData = this.tblDataCopy.filter(
      x => x.code.toLowerCase().includes(name.toLowerCase()) ||
        x.code.toLowerCase().includes(name.toLowerCase()) 
  ...
  /*filter() {
    ...
    this.manager_Override_ListFilter = this.manager_Override_ListFilter;

    if (code != null && code != "")
      this.manager_Override_ListFilter = this.manager_Override_ListFilter.filter(x => x.code.toLowerCase().includes(code.toLowerCase()));

    if (name != null && name != "")
      this.manager_Override_ListFilter = this.manager_Override_ListFilter.filter(x => x.name.toLowerCase().includes(name.toLowerCase()));
