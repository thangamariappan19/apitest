...
@Component({
  selector: 'app-price-change-add',
  templateUrl: './price-change-add.component.html',
  styleUrls: ['./price-change-add.component.css']
})
export class PriceChangeAddComponent implements OnInit {
  myForm: FormGroup;
  ...
  createForm() {
    ...
    this.getCountryList();
    this.myForm.get('sourceCountry').setValue('');
      ...
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            ...
            for(let count of this.countryList)
            {
              count.active=false;
            }
        ...
        if (selCountry == true) {
          ...
          for (let i = 0; i < this.columnList.length; i++) {
            ...
            let tempcountry: MPriceChangeCountries = {
              ...
              currencyCode: this.currencyList[i].currencyCode
            }
  ...
  getActualValue(col_name: string, serialNo: number) {
    ...
    if (col_name == "Remarks") {
      ...
      if (this.validateStyleList.length > 0) {
        for (let serial of this.styleList) {
          let arr = this.priceChangeDetailsList.filter(x => x.style_serialNo == serial.serialNo)
        ...
        if (stList != null && stList.length > 0) {
          ...
          if (st != null) {
            if (column_name == "StyleCode") {
              ...
              col_value = st.BrandName == null ? "" : st.BrandName;
  ...
  savePriceChange() {
    if (this.isGenerate == false || this.priceChangeDetailsList.length == 0) {
      ...
      if (this.priceChangeHeader == null) {
        ...
        this.priceChangeHeader.remarks=this.myForm.get('remarks').value;
  ...
  clear() {
    this.myForm.controls['sourceCountry'].setValue('');
    //this.myForm.controls['documentDate'].setValue('');
    //this.myForm.controls['priceChangeDate'].setValue('');
    this.myForm.controls['priceChangeType'].setValue('');
    this.myForm.controls['multipleCountries'].setValue(false);
    this.myForm.controls['remarks'].setValue('');
    this.tableColumns = [];
    this.styleList = [];
    this.priceChangeDetailsList = [];
    this.priceChangeCountries = [];
    this.countryDropDownList = [];
    this.gridcolumnheaders = [];
    this.myForm.get('documentNo').enable();
    this.myForm.get('documentDate').enable();
    this.myForm.get('priceChangeDate').enable();
    this.myForm.get('priceChangeType').enable();
    this.myForm.get('sourceCountry').enable();
    this.myForm.get('multipleCountries').enable();
  }
