...
export class YearListComponent implements OnInit {
  ...
  json: Array<any>;
  year_List: Array<MYearMaster>;
  year_Filter: Array<MYearMaster>;
  myForm: FormGroup;
  ...
  private getData(): void {
    ...
    this.api.getAPI("year?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
  ...
  clear_controls(){
    ...
    this.year_Filter = this.year_List;
  }
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'yearCode', title: 'Year Code', width: '30%' },
      { key: 'year', title: 'Year Name', width: '30%' },      
  ...
  onChange(name: string): void {
    this.tblData = this.tblDataCopy.filter(
      x => x.yearCode.toLowerCase().includes(name.toLowerCase()) ||
        x.year.toLowerCase().includes(name.toLowerCase()) ||
