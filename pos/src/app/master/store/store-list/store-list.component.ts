...
export class StoreListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("store?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.storeMasterList;
...
dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'storeCode', title: 'Store Code', width: '15%' }, // orderBy: 'desc' },
      { key: 'storeName', title: 'Store Name', width: '15%' },
      { key: 'countryName', title: 'Country', width: '10%' },
      { key: 'storeGroupName', title: 'Store Group', width: '10%' },
      { key: 'companyName', title: 'Company', width: '10%' },
      { key: 'brand', title: 'Brand', width: '10%' },
      { key: 'storeType', title: 'StoreType', width: '10%' },
  ...
  //     x => x.storeCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.storeName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.countryName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.storeGroupName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.companyName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.brand.toLowerCase().includes(name.toLowerCase()) ||
  //       x.storeType.toLowerCase().includes(name.toLowerCase()) ||
  ...
  //   this.store = null;
  //   this.common.showSpinner();
  //   this.api.getAPI("store")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           this.store = data.storeMasterList;
  //           //this.json = data.companySettingList;
  //           //.log(this.json);
  //           this.tblData = data.storeMasterList;
  //           this.tblDataCopy = data.storeMasterList;
  //         } else {
  //           let msg: string = data != null
  //             && data.displayMessage != null
  //             && data.displayMessage != "" ? data.displayMessage : "Failed to retrieve Data.";
