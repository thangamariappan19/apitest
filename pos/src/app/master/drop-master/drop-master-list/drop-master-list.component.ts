...
@Component({
  selector: 'app-drop-master-list',
  templateUrl: './drop-master-list.component.html',
  styleUrls: ['./drop-master-list.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DropMasterListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("drop?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'dropCode', title: 'Drop Code', width: '20%' }, // orderBy: 'desc' },
      { key: 'dropName', title: 'Drop Name', width: '30%' },
      { key: 'remarks', title: 'Remarks', width: '30%' },
  ...
  //   this.drop_Filter = this.drop_list;
  // }

  // getdropList() {
  //   this.json = null;
  //   this.drop_list = null;
  //   this.drop_Filter = null;
  //   this.common.showSpinner();
  //   this.common.showSpinner();
  ...
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           // this.drop_list = new Array<MDropMaster>();
  ...
  //     x => x.dropCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.dropName.toLowerCase().includes(name.toLowerCase()) ||
  ...
  // }
  // // filter() {
  // //   let dropCode = this.myForm.get('dropCode').value;
  // //   let dropName = this.myForm.get('dropName').value;
  // //   let remarks = this.myForm.get('remarks').value;

  // //   this.drop_Filter = this.drop_list;

  // //   if (dropCode != null && dropCode != "")
  // //     this.drop_Filter = this.drop_Filter.filter(x => x.dropCode.toLowerCase().includes(dropCode.toLowerCase()));

  // //   if (dropName != null && dropName != "")
  // //     this.drop_Filter = this.drop_Filter.filter(x => x.dropName.toLowerCase().includes(dropName.toLowerCase()));

  // //   if (remarks != null && remarks != "")
  // //     this.drop_Filter = this.drop_Filter.filter(x => x.remarks.toLowerCase().includes(remarks.toLowerCase()));
  // // }
  // changeGender(e) {
  //   //alert(e.target.value);
  //   if(e.target.value=='no')
