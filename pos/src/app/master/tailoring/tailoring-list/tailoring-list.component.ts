...
export class TailoringListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("tailoring?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.tailoringMasterList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'tailoringunitcode', title: 'Tailoring Code', width: '25%' }, // orderBy: 'desc' },
      { key: 'tailoringunitName', title: 'Tailoring Name', width: '35%' },
      { key:'countryCode',title:'Country Code',width:'20%'},
      { key:'storeCode',title:'Store Code',width:'20%'},
  ...
  //     x => x.tailoringunitcode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.tailoringunitName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.countryCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.storeCode.toLowerCase().includes(name.toLowerCase())
  //   );
  // }

  // getTailoringList() {
  //   this.tailoringList = null;
  //   this.common.showSpinner();
  //   this.api.getAPI("tailoring")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           //this.json = new Array<MCountryMaster> ();
  //          // this.tailoringList = data.tailoringMasterList;
  //           // this.json = data.countryMasterList;
  //           //// .log(this.json);
  //           this.tblData = data.tailoringMasterList;
  //           this.tblDataCopy = data.tailoringMasterList;
  //         } else {
  //           let msg: string = data != null
  //             && data.displayMessage != null
  //             && data.displayMessage != "" ? data.displayMessage : "Failed to retrieve Data.";
