...
@Component({
  selector: 'app-coupon-receipt-list',
  templateUrl: './coupon-receipt-list.component.html',
  styleUrls: ['./coupon-receipt-list.component.css']
})
export class CouponReceiptListComponent implements OnInit {
  myForm: FormGroup;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'couponCode', title: 'Coupon Code', width: '45%' }, // orderBy: 'desc' },
      { key: 'currentLocation', title: 'Current Location', width: '45%' },
  ...
  private getData(): void {
    this.common.showSpinner();
    this.tblData = null;
    this.api.getAPI("CouponReceipt?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.responseDynamicData;
