...
export class ProductLineListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("productline?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.productLineMasterList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key: 'productLineCode', title: 'Product Line Code', width: '30%' },
      { key: 'productLineName', title: 'Product Line Name', width: '30%' },      
  ...
  //     x => x.productLineCode.toLowerCase().includes(name.toLowerCase()) ||
  //       x.productLineName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.description.toLowerCase().includes(name.toLowerCase())
  //   );
  // }
  // getProductGroup(){
  //   this.productline = null;
  //   this.productline_Filter=null;
  //   this.common.showSpinner();
  //   this.api.getAPI("productline")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //           //this.productline = data.productLineMasterList;
  //           //this.productline_Filter = this.productline;
  //           this.tblData=data.productLineMasterList;
  //           this.tblDataCopy=data.productLineMasterList;          
  //         } else {
