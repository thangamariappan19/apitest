...
@Component({
  ...
  styleUrls: ['./document-numbering-list.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DocumentNumberingListComponent implements OnInit {
  ...
  private getData(): void {
    ...
    this.api.getAPI("DocumentNumbering?limit=" + this.pagination.limit + "&offset=" + this.pagination.offset + "&isActive=" + this.isActive + "&searchString=" + this.searchString)
      .subscribe((data) => {
        setTimeout(() => {
          if (data != null && data.statusCode != null && data.statusCode == 1) {
            this.tblData = data.documentNumberingMasterList;
  ...
  dataTableDefault() {
    ...
    this.columnsCopy = [
      { key:'country', title: 'Country', width: '30%' }, // orderBy: 'desc' },
      { key:'state', title: 'State', width: '30%' },
      { key:'store',title:'Store',width:'20%'},
      { key:'documentType',title:'Document Type',width:'20%'},
      { key: '', title: '' },
    ];
  ...
  //     x => x.countryName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.stateName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.storeName.toLowerCase().includes(name.toLowerCase()) ||
  //       x.documentName.toLowerCase().includes(name.toLowerCase()) 
       
  //   );
  // }

  // getDocumentNumberingMasterList() {
  //   this.documentNumberingList = null;
  //   this.documentNumberingFilter = null;
  //   this.common.showSpinner();
  //   this.api.getAPI("DocumentNumbering")
  //     .subscribe((data) => {
  //       setTimeout(() => {
  //         if (data != null && data.statusCode != null && data.statusCode == 1) {
  //          // this.documentNumberingList = data.documentNumberingMasterList;
  //          // this.documentNumberingFilter = this.documentNumberingList;
  //           this.tblData = data.documentNumberingMasterList;
  //           this.tblDataCopy = data.documentNumberingMasterList;
  //         } else {
